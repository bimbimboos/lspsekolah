...
function cek_csrf() {
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        die('<p style="font-family:sans-serif;padding:40px;color:#dc2626;">403 - Token tidak valid. <a href="data_kontak.php">Kembali</a></p>');
    }
}
...
if (isset($_POST['tambah'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { set_flash('danger','Format email tidak valid.'); header("Location: data_kontak.php"); exit; }
    if (empty($nama) || empty($pesan)) { set_flash('danger','Nama dan pesan tidak boleh kosong.'); header("Location: data_kontak.php"); exit; }
    $email   = mysqli_real_escape_string($koneksi, $email);
    $tanggal = date('Y-m-d');
    mysqli_query($koneksi, "INSERT INTO kontak (nama, email, pesan, tanggal_kirim) VALUES ('$nama','$email','$pesan','$tanggal')");
    set_flash('success','Pesan berhasil ditambahkan.');
    header("Location: data_kontak.php"); exit;
}
...
if (isset($_POST['hapus'])) {
    ...
    if ($id > 0) { mysqli_query($koneksi, "DELETE FROM kontak WHERE id=$id"); set_flash('success','Pesan berhasil dihapus.'); }
    header("Location: data_kontak.php"); exit;
}
...
if (isset($_POST['edit'])) {
    ...
    $pesan = trim(mysqli_real_escape_string($koneksi, $_POST['pesan']));
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { set_flash('danger','Format email tidak valid.'); header("Location: data_kontak.php"); exit; }
    if (empty($nama) || empty($pesan) || $id <= 0) { set_flash('danger','Data tidak lengkap.'); header("Location: data_kontak.php"); exit; }
    $email = mysqli_real_escape_string($koneksi, $email);
...
/* ============================================================
   BALAS PESAN USER  ← FITUR BARU
   ============================================================ */
if (isset($_POST['balas'])) {
    cek_csrf();
    $id      = (int)$_POST['id'];
    $balasan = trim(mysqli_real_escape_string($koneksi, $_POST['balasan']));
    if (empty($balasan) || $id <= 0) { set_flash('danger','Balasan tidak boleh kosong.'); header("Location: data_kontak.php"); exit; }
    $now = date('Y-m-d H:i:s');
    // Tandai sudah_dibaca = 0 agar user mendapat notif baru
    mysqli_query($koneksi, "UPDATE kontak SET balasan='$balasan', tanggal_balas='$now', sudah_dibaca=0 WHERE id=$id");
    set_flash('success','Balasan berhasil dikirim ke user.');
    header("Location: data_kontak.php"); exit;
}

/* ============================================================
   HAPUS BALASAN  ← FITUR BARU
   ============================================================ */
if (isset($_POST['hapus_balasan'])) {
    cek_csrf();
    $id = (int)$_POST['id'];
    if ($id > 0) {
        mysqli_query($koneksi, "UPDATE kontak SET balasan=NULL, tanggal_balas=NULL, sudah_dibaca=0 WHERE id=$id");
        set_flash('success','Balasan berhasil dihapus.');
    }
    header("Location: data_kontak.php"); exit;
}
...
$filter_status = isset($_GET['filter_status']) ? $_GET['filter_status'] : ''; // 'dibalas' | 'belum'
...
if ($filter_status === 'dibalas') $where .= " AND balasan IS NOT NULL AND balasan != ''";
if ($filter_status === 'belum')   $where .= " AND (balasan IS NULL OR balasan = '')";

/* ============================================================
   PAGINATION
   ============================================================ */
$limit     = isset($_GET['limit']) ? (int)$_GET['limit'] : 5;
$limit     = in_array($limit,[5,10,25,50]) ? $limit : 5;
...
$total_dibalas = mysqli_num_rows(mysqli_query($koneksi, "SELECT id FROM kontak WHERE balasan IS NOT NULL AND balasan != ''"));
$total_belum   = mysqli_num_rows(mysqli_query($koneksi, "SELECT id FROM kontak WHERE balasan IS NULL OR balasan = ''"));
...
.stat-card{flex:1;min-width:120px;background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:14px 18px;box-shadow:var(--shadow);display:flex;align-items:center;gap:14px}
.stat-icon{width:44px;height:44px;border-radius:10px;display:grid;place-items:center;font-size:1.2rem;flex-shrink:0}
.stat-icon.blue{background:#eff6ff;color:var(--primary)}.stat-icon.green{background:#ecfdf5;color:var(--success)}.stat-icon.amber{background:#fffbeb;color:var(--accent)}.stat-icon.red{background:#fff1f2;color:var(--danger)}
.stat-val{font-size:1.5rem;font-weight:700;color:var(--dark);line-height:1}.stat-lbl{font-size:.75rem;color:var(--muted);margin-top:3px}
...
.filter-select{height:36px!important;font-size:.85rem!important;border-radius:var(--radius-sm)!important;border-color:var(--border)!important;min-width:140px}
...
.pesan-preview{max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.82rem;color:var(--muted)}
...
.badge-dibalas{font-size:.72rem;background:#ecfdf5;color:#059669;padding:3px 9px;border-radius:20px;font-weight:700;display:inline-flex;align-items:center;gap:3px}
.badge-belum{font-size:.72rem;background:#fffbeb;color:#d97706;padding:3px 9px;border-radius:20px;font-weight:700;display:inline-flex;align-items:center;gap:3px}
.badge-bukan-user{font-size:.72rem;background:#f1f5f9;color:#94a3b8;padding:3px 8px;border-radius:20px;font-weight:600}
...
.btn-act.balas{background:#ecfdf5;color:#059669}.btn-act.balas:hover{background:#059669;color:#fff}
...
.balasan-preview-box{background:#ecfdf5;border:1px solid #a7f3d0;border-radius:8px;padding:12px 14px;font-size:.85rem;color:#065f46;white-space:pre-wrap;margin-top:10px}
.balasan-preview-head{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#059669;margin-bottom:6px;display:flex;align-items:center;gap:6px}
...
/* Badge user kiriman */
.badge-from-user{font-size:.7rem;background:#eff6ff;color:#1e40af;padding:2px 8px;border-radius:20px;font-weight:700;border:1px solid #bfdbfe}
</style>
...
<div class="container-fluid py-3">
  ...
  <div class="pg-header">
    <div class="pg-title">
      ...
      <div>Data Kontak<small>Pesan masuk & balasan ke pengunjung/user</small></div>
    </div>
  ...
  <div class="stat-row">
    ...
    <div class="stat-card">
      ...
      <div><div class="stat-val"><?= $total_dibalas ?></div><div class="stat-lbl">Sudah Dibalas</div></div>
    </div>
    <div class="stat-card">
      <div class="stat-icon amber"><i class="bi bi-clock-fill"></i></div>
      <div><div class="stat-val"><?= $total_belum ?></div><div class="stat-lbl">Belum Dibalas</div></div>
    </div>
    <div class="stat-card">
  ...
  <div class="toolbar">
    <div class="toolbar-left">
      ...
      <!-- Filter Status Balasan (BARU) -->
      <select name="filter_status" class="form-select filter-select" onchange="this.form.submit()">
        <option value="">Semua Status</option>
        <option value="dibalas" <?= $filter_status==='dibalas'?'selected':'' ?>>✓ Sudah Dibalas</option>
        <option value="belum"   <?= $filter_status==='belum'  ?'selected':'' ?>>⏳ Belum Dibalas</option>
      </select>
    ...
    <div class="toolbar-right">
      <?php if ($search || $filter_bulan || $filter_status): ?>
  ...
  <div class="tbl-card">
    ...
    <table class="table table-hover mb-0">
      ...
      <?php if (mysqli_num_rows($data) == 0): ?>
        <tr><td colspan="7">
          <div class="empty-state"><i class="bi bi-inbox"></i><p>Tidak ada pesan ditemukan.</p></div>
        </td></tr>
      <?php else: $no = $offset + 1; while ($d = mysqli_fetch_assoc($data)): ?>
        <tr>
          ...
          <td>
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <div>
                <div style="font-weight:600;color:var(--dark)"><?= htmlspecialchars($d['nama']) ?></div>
                <?php if ($d['id_user']): ?>
                  <span class="badge-from-user"><i class="bi bi-person-fill me-1"></i>User Terdaftar</span>
                <?php endif; ?>
              </div>
            </div>
          </td>
          ...
          <td style="text-align:center">
            <?php if (!empty($d['balasan'])): ?>
              <span class="badge-dibalas"><i class="bi bi-check-circle-fill"></i> Dibalas</span>
            <?php elseif ($d['id_user']): ?>
              <span class="badge-belum"><i class="bi bi-clock-fill"></i> Menunggu</span>
            <?php else: ?>
              <span class="badge-bukan-user">Umum</span>
            <?php endif; ?>
          </td>
          <td style="text-align:center">
            <!-- Lihat -->
            ...
            <!-- Balas (hanya jika dari user terdaftar) -->
            <?php if ($d['id_user']): ?>
            <button class="btn-act balas" data-bs-toggle="modal" data-bs-target="#balasKontak<?= $d['id'] ?>" title="Balas Pesan">
            ...
            <!-- Hapus -->
            <button class="btn-act hapus ms-1" title="Hapus" onclick="konfirmasiHapus(<?= $d['id'] ?>,'<?= addslashes(htmlspecialchars($d['nama'])) ?>')">
          ...
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="d-flex align-items-center gap-3 mb-3 p-3" style="background:var(--primary-xs);border-radius:8px">
                  ...
                  <div>
                    ...
                    <?php if ($d['id_user']): ?><span class="badge-from-user mt-1 d-inline-block">User Terdaftar</span><?php endif; ?>
                  </div>
                </div>
                ...
                <?php if (!empty($d['balasan'])): ?>
                <div class="mt-3">
                  <div class="balasan-preview-head"><i class="bi bi-reply-fill"></i> Balasan Admin</div>
                  <div class="balasan-preview-box"><?= htmlspecialchars($d['balasan']) ?></div>
                  <?php if ($d['tanggal_balas']): ?>
                    <div style="font-size:.75rem;color:var(--muted);margin-top:5px;"><i class="bi bi-clock me-1"></i>Dibalas: <?= date('d M Y H:i', strtotime($d['tanggal_balas'])) ?></div>
                  <?php endif; ?>
                </div>
                <?php endif; ?>
              </div>
              <div class="modal-footer">
                ...
                <a href="mailto:<?= htmlspecialchars($d['email']) ?>" class="btn btn-primary btn-sm"><i class="bi bi-envelope me-1"></i>Email Langsung</a>
                <?php if ($d['id_user']): ?>
                  <button type="button" class="btn btn-success btn-sm" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#balasKontak<?= $d['id'] ?>"><i class="bi bi-reply-fill me-1"></i>Balas di Sistem</button>
                <?php endif; ?>
              </div>
            </div>
          </div>
        ...
        <!-- ================================================
             MODAL BALAS PESAN (FITUR BARU)
             ================================================ -->
        <?php if ($d['id_user']): ?>
        <div class="modal fade" id="balasKontak<?= $d['id'] ?>" tabindex="-1">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <form method="POST" action="data_kontak.php">
                ...
                <div class="modal-header">
                  <h5 class="modal-title"><i class="bi bi-reply-fill me-2 text-success"></i>Balas Pesan User</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                  <!-- Info pengirim -->
                  <div class="d-flex align-items-center gap-3 mb-3 p-3" style="background:var(--surface);border:1px solid var(--border);border-radius:8px">
                    <div class="sender-avatar"><?= strtoupper(substr($d['nama'],0,1)) ?></div>
                    <div>
                      <div style="font-weight:600;font-size:13.5px;color:var(--dark)"><?= htmlspecialchars($d['nama']) ?></div>
                      <div style="font-size:.78rem;color:var(--muted)"><?= htmlspecialchars($d['email']) ?></div>
                    </div>
                  </div>
                  <!-- Pesan asli -->
                  <label class="form-label">Pesan dari User:</label>
                  <div class="pesan-box mb-3"><?= htmlspecialchars($d['pesan']) ?></div>
                  <!-- Form balasan -->
                  <label class="form-label">Balasan Admin <span class="text-danger">*</span></label>
                  <textarea name="balasan" class="form-control" rows="5"
                    placeholder="Tulis balasan kamu di sini. User akan melihat balasan ini di halaman kontak mereka."
                    required maxlength="2000"><?= htmlspecialchars($d['balasan'] ?? '') ?></textarea>
                  <div style="font-size:11px;color:var(--muted);margin-top:5px;"><i class="bi bi-info-circle me-1"></i>Balasan akan langsung terlihat oleh user saat mereka membuka halaman kontak.</div>
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
                  <?php if (!empty($d['balasan'])): ?>
                  <form method="POST" action="data_kontak.php" style="display:inline;" onsubmit="return confirm('Hapus balasan ini?')">
                    <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
                    <input type="hidden" name="id" value="<?= $d['id'] ?>">
                    <button type="submit" name="hapus_balasan" class="btn btn-outline-danger btn-sm"><i class="bi bi-x-circle me-1"></i>Hapus Balasan</button>
                  </form>
                  <?php endif; ?>
                  <button type="submit" name="balas" class="btn btn-success btn-sm"><i class="bi bi-reply-fill me-1"></i><?= !empty($d['balasan']) ? 'Update Balasan' : 'Kirim Balasan' ?></button>
                </div>
              </form>
            </div>
          </div>
    ...
    <div class="pagi-wrap">
      ...
      <?php if ($totalPage > 1): ?>
      <ul class="pagination">
        <li class="page-item <?= $page==1?'disabled':'' ?>"><a class="page-link" href="?page=<?= $page-1 ?>&search=<?= urlencode($search) ?>&filter_bulan=<?= urlencode($filter_bulan) ?>&filter_status=<?= urlencode($filter_status) ?>&limit=<?= $limit ?>"><i class="bi bi-chevron-left"></i></a></li>
        <?php for($i=1;$i<=$totalPage;$i++): ?><li class="page-item <?= $i==$page?'active':'' ?>"><a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&filter_bulan=<?= urlencode($filter_bulan) ?>&filter_status=<?= urlencode($filter_status) ?>&limit=<?= $limit ?>"><?= $i ?></a></li><?php endfor; ?>
        <li class="page-item <?= $page==$totalPage?'disabled':'' ?>"><a class="page-link" href="?page=<?= $page+1 ?>&search=<?= urlencode($search) ?>&filter_bulan=<?= urlencode($filter_bulan) ?>&filter_status=<?= urlencode($filter_status) ?>&limit=<?= $limit ?>"><i class="bi bi-chevron-right"></i></a></li>
      </ul>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form method="POST" action="data_kontak.php">
        ...
        <div class="modal-body">
          <div class="mb-3"><label class="form-label">Nama <span class="text-danger">*</span></label><input type="text" name="nama" class="form-control" placeholder="Nama pengirim" required maxlength="100"></div>
          <div class="mb-3"><label class="form-label">Email <span class="text-danger">*</span></label><input type="email" name="email" class="form-control" placeholder="email@contoh.com" required maxlength="150"></div>
          <div class="mb-3"><label class="form-label">Pesan <span class="text-danger">*</span></label><textarea name="pesan" class="form-control" rows="4" placeholder="Tulis pesan..." required maxlength="2000"></textarea></div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Batal</button>
    ...
    <div class="modal-content">
      <form method="POST" action="data_kontak.php" id="formHapus">
        ...
        <div class="modal-body text-center" style="padding:28px 22px!important">
          ...
          <p style="font-size:.85rem;color:var(--muted);margin-bottom:20px">Pesan dari <b id="hapusNamaLabel">-</b> akan dihapus permanen.</p>
          <div class="d-flex gap-2 justify-content-center">
            <button type="button" class="btn btn-secondary btn-sm px-4" data-bs-dismiss="modal">Batal</button>
            <button type="submit" class="btn btn-danger btn-sm px-4"><i class="bi bi-trash me-1"></i>Ya, Hapus</button>
          </div>
        </div>
      </form>
    </div>
...
<script>
let searchTimer;
function autoSubmit() { clearTimeout(searchTimer); searchTimer = setTimeout(()=>document.getElementById('filterForm').submit(),500); }
